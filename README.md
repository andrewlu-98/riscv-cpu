...
To visualize what a B-type instruction does, the LEDs are remapped to the Program Counter (PC_top[7:2]), showing the current instruction address being executed. Upon hitting the beq x9, x9, 12 instruction, the ALU compares the two registers and evaluates the condition as true. We see that the value jumps from 16 (00010000, 2 least significant bits not shown in video) to 40 (00101000), which is due to the shift left 1 block. 
